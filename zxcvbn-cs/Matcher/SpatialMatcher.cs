...
namespace Zxcvbn.Matcher
{
    ...
    {
        private const string SpatialPattern = "Spatial Pattern on Keyboard:{0}";

        private readonly Lazy<List<SpatialGraph>> _spatialGraphs = new Lazy<List<SpatialGraph>>(GenerateSpatialGraphs);

        /// <inheritdoc />
        /// <summary>
        /// PasswordMatch the password against the known keyboard layouts
        /// </summary>
...
";
        ...
        /// <summary>
        /// PasswordMatch the password against a single pattern
        /// </summary>
        /// <param name="graph">Adjacency graph for this key layout</param>
        /// <param name="password">Password to match</param>
        /// <returns>List of matching patterns</returns>
        private static IEnumerable<PasswordMatch> SpatialMatch(SpatialGraph graph, string password)
        {
            ...
            while (i < password.Length - 1)
            {
                ...
                if (j - i > 2)
                {
                    ...
                    {
                        Pattern = string.Format(SpatialPattern, graph.Name),
                        Begin = i,
                        End = j - 1,
