...
namespace Zxcvbn
{
    ...
    public class PasswordMetric
    {
        ...
        /// <returns>PasswordMetricResult for lowest entropy match</returns>
        public PasswordMetricResult EvaluatePassword(string password, IEnumerable<string> userInputs = null)
        {
            ...
            IEnumerable<PasswordMatch> matches = new List<PasswordMatch>();
        ...
        private PasswordMetricResult FindMinimumEntropyMatch(string password, IEnumerable<PasswordMatch> matches)
        {
            ...
            var bestMatchForIndex = new PasswordMatch[password.Length];
            ...
            var matchSequence = new List<PasswordMatch>();
            ...
            if (matchSequence.Count == 0)
            {
                ...
                matchSequence.Add(new PasswordMatch()
                {
                    Begin = 0,
                    End = password.Length,
                    Token = password,
            ...
            else
            {
                ...
                var matchSequenceCopy = new List<PasswordMatch>();
                for (var k = 0; k < matchSequence.Count; k++)
                {
                    var m1 = matchSequence[k];
                    var m2 = (k < matchSequence.Count - 1 ? matchSequence[k + 1] : new PasswordMatch() { Begin = password.Length }); // Next match, or a match past the end of the password
                    ...
                    if (m1.End < m2.Begin - 1)
                    {
                        ...
                        var ns = m1.End + 1;
                        var ne = m2.Begin - 1;
                        matchSequenceCopy.Add(new PasswordMatch()
                        {
                            Begin = ns,
                            End = ne,
