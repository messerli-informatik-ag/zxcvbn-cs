...
namespace Zxcvbn.Matcher
{
    ...
    {
        private const string DictionaryPattern = "Dictionary '{0}'";

        private readonly string _dictionaryName;
        private readonly Lazy<Dictionary<string, int>> _rankedDictionary;

        /// <summary>
        /// Creates a new dictionary matcher. <paramref name="resourcePath"/> must be the path (relative or absolute) to a file containing one word per line,
        /// entirely in lowercase, ordered by frequency (decreasing); or <paramref name="resourcePath"/> must be the name of a built-in dictionary.
        /// </summary>
        /// <param name="name">The name provided to the dictionary used</param>
        /// <param name="resourcePath">The filename of the dictionary (full or relative path) or name of built-in dictionary</param>
        public DictionaryMatcher(string name, string resourcePath)
        {
            _dictionaryName = name;
            _rankedDictionary = new Lazy<Dictionary<string, int>>(() => BuildRankedDictionary(resourcePath));
        }
        ...
        /// <inheritdoc />
        /// <summary>
        /// PasswordMatch substrings of password agains the loaded dictionary
        /// </summary>
        ...
        public IEnumerable<PasswordMatch> MatchPassword(string password)
        {
            ...
            var matches = (from i in Enumerable.Range(0, password.Length)
                           ...
                           {
                               Pattern = string.Format(DictionaryPattern, _dictionaryName),
                               Begin = i,
                               End = j,
        ...
        private static Dictionary<string, int> BuildRankedDictionary(string resourcePath)
        {
            // Look first to wordlists embedded in assembly (i.e. default dictionaries) otherwise treat as file path
            var lines = Utility.GetEmbeddedResourceLines(resourcePath);
